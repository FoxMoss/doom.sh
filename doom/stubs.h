#define TUNE_MAXVISPLANES                                                      \
  32 // At 16, frequently causes visual glitches, but size is critical.
...
#define COMBINE_SCREENS // Overlap memory for screens #0-3, saves RAM would mess
                        // up wipes, but we don't care.

// Disable screen wipings, saves peaks of 64k on heap.
#define DISABLE_WIPES

// We can create tables for things like the textures and maps... You can set the
// flags here to produce these. Don't do this on target hardware!!!

#ifndef FIXED_HEAP
#ifdef GENERATE_BAKED
#define FIXED_HEAP                                                             \
  40000000 // Heap size when running on computer to store full size.
#else
